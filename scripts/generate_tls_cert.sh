...
# Generate short-lived demo certificates for mutual TLS.  The server cert is
# tied to SERVER_IP so kbmon_tls can verify it during the TLS handshake.
if [[ $# -lt 1 ]]; then
  echo "Usage: $0 SERVER_IP"
  echo "Example: $0 192.168.1.20"
  echo "Do not include angle brackets around SERVER_IP."
  exit 1
fi
...
# Client certificate: identifies the Pi to the server for mutual TLS.
